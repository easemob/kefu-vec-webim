...
class Ws {
    ...
    handleMessage = (msg, options) => {
        ...
        // 尝试
        if(officialAccount){
            this.attemptToAppendOfficialAccount(officialAccount);
        }
        targetOfficialAccount = this._getOfficialAccountById(officialAccountId);
        ...
        if(!isHistory){
            // 实时消息需要处理系统事件
            if(eventName){
                this._handleSystemEvent(eventName, eventObj, msg);
            }
            else{
    ...
    _handleSystemEvent = (eventName, eventObj, msg) => {
        ...
        var officialAccount = this._getOfficialAccountById(officialAccountId);
    ...
    _initConnection = () => {
        ...
        this.state.conn.listen({
            onOpened: function(info){
                ...
                this.state.conn.setPresence();
            },
            onTextMessage: function(message){
                this.handleMessage(message, { type: "txt", notFromSystem: true });
            },
            onPictureMessage: function(message){
                this.handleMessage(message, { type: "img" });
            },
            onFileMessage: function(message){
                this.handleMessage(message, { type: "file" });
            },
            onVideoMessage: function(message){
                this.handleMessage(message, { type: "video" });
            }, // 新增小视频类型
    ...
    sendText = (message, ext = {}) => {
        ...
        this._setExt(msg);
        this.state.conn.send(msg.body);
    }
    
    // 挂断视频
    cancelVideo = (callId, ext) => {
        var id = utils.uuid();
